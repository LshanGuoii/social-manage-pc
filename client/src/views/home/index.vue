 ...
 <div class="index">
    ...
    <el-table-column
      ...
      >
       <template slot-scope="scope">
            ...
            <div>{{ scope.row.weChatNameList}}</div>
              <div slot="content">{{scope.row.weChatNameList}}</div>
          </el-tooltip>
      </template>
    </el-table-column>
     <el-table-column
  ...
  <div class="index-page">
    <q-page :page="page" @loadData="loadData"></q-page>
      ...
      <add-group v-model="showAddTemp" :detail="detail" @updatData="loadData()"></add-group>
   </div>
 </div>
</template>
      ...
      this.apis.getRulesFind(params).then((res) => {
        if (res.code === 0) {
          ...
          this.tableData.forEach((item) => {
            let weChatNameList = []
            item.group_ids = JSON.parse(item.group_ids)
            this.page.total = res.data.total
            if (item.group_ids.length !== 0) {
              item.group_ids.forEach((name) => {
                weChatNameList.push(name.name)
              })
            }
            let string = []
            string = weChatNameList.join(' , ')
            item.weChatNameList = string
            // this.$set(item, 'weChatNameList', string)
          })
        }
      }).catch((err) => {
